...
public class StockR implements StockReader {
	...
	public synchronized String searchProducts(String searchTerm) throws StockException {
		try {
			String q = String.format("select productNo from ProductTable where LOWER(ProductTable.description) LIKE \'%%%s%%\'", searchTerm.toLowerCase());
			DEBUG.trace("QUERY: %s", q);
			ResultSet rs = getStatementObject().executeQuery(q);
			
			
			if (rs.next()) {
				String result = rs.getString(1);
				DEBUG.trace("Result: %s", result);
				rs.close();
				return result;
			} else {
				rs.close();
				return String.format("Prodict: %s can't be found", searchTerm);
			}
